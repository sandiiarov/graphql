...
beforeEach(() => {
  BookingComApiMock.onGet(
    'https://distribution-xml.booking.com/2.0/json/hotelAvailability?extras=hotel_details&order_by=popularity&city_ids=-2140205&checkin=2017-11-16&checkout=2017-11-23&currency=CZK&rows=2&room1=A',
  ).replyWithData(AllHotelsDataset);
});

describe('all hotels query', () => {
  it('should work with specified currency option', async () => {
    expect(
      await graphql(`
        {
          allAvailableHotels(
